...
\title{Documentation -- Hybridization Expansion CT-QMC solver}
\author{Emanuel Gull}
...
The hybridization solver is built with the ALPS applications as the executable \verb#hybridization#. Follow the current build instructions in \verb#CONTRIBUTING.md# at the repository root; the adjacent \verb#README.md# links to the build guide and Python tutorials. Source builds require CMake 4.3 or newer and the documented ALPS dependencies. Configure with \verb#ALPS_BUILD_APPLICATIONS=ON# and build the \verb#hybridization# and \verb#p2h5# targets.

MPI is optional. Serial builds and Python solver calls require no MPI initialization. For MPI-enabled native runs, build the SDK with \verb#ALPS_ENABLE_MPI=ON# and use the MPI launcher provided by your installation.

The standalone pyalps package supplies the Python solver interface. Its default build includes application bindings and requires an installed SDK with solver libraries; see the package's own build instructions.
...
BETA = 45
\end{verbatim}
This is a parameter file for a single impurity Anderson model (two ``orbitals'', i.e. $N_{\text{orb}}=2$, one for spin ``up'' and one for spin ``down'') with $U=4$, $\mu=2$, and $\beta=45$ for which the hybridization function will be read from the file \verb#delta.dat#. The energy units are the ones of \verb#delta.dat#. This simulation runs for sixty seconds.
...
\begin{verbatim}
$ALPS_PREFIX/bin/hybridization hyb1.h5
\end{verbatim}
...
The first line imports the solver module. The script writes a simple constant hybridization function to file. Serial solver calls do not require the optional Python MPI extra.
...
\end{verbatim}
Use the MPI-enabled standalone executable for a distributed native run.

The above example and more advanced examples on how to use the Python framework can be found in the hybridization tutorials within the \verb#share/alps/tutorials# directory inside the ALPS installation prefix (installed with the optional tutorials component):
\begin{verbatim}
...
Using python together with the Python interface of the solver (the cthyb Python module) provides a very flexible framework for setting up any desired selfconsistency scheme.
...
\begin{itemize}
\item Dynamical mean field theory: please have a look at the original review\cite{Georges96} and the review by Kotliar {\it et al.}\cite{Kotliar06}.
