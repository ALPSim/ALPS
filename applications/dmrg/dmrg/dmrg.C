...
DMRGTask::DMRGTask(const alps::ProcessList& w,const boost::filesystem::path& fn)
  ...
  , alps::EigenvectorMeasurements<double>(*this)
{
  init();
}
...
//-------------------------------------------------------------
  ...
  dmtk::Hami<double> meas_terms(hami);
  meas_terms.clear();
...
//meas.set_name(meas.description().c_str());
        meas_terms += dmtk::BasicOp<double>(meas);
      }
    }
    else {
...
//meas.set_name(meas.description().c_str());
  ...
  BOOST_FOREACH (string_string_pair_pair const& ex, this->correlation_expressions) {
    std::vector<dmtk::Hami<double> > corr_meas(num_distances());
    ...
    for (unsigned d=0; d<num_distances();++d) {
      ...
      meas_terms += dmtk::BasicOp<double>(corr_meas[d]);
    }
  }

  dmtk::Hami<double>::iterator iter;
  int i = 0;
...
{
  dmtk::System<double> &S = this->system;
  dmtk::Hami<double>::iterator iter = S.corr.begin();
  typedef std::pair<std::string,std::string> string_pair;
  typedef std::pair<std::string,std::pair<std::string,std::string> > string_string_pair_pair;
  ...
  // average measurements will be identical loops, but all terms added instead of stored separately
   ...
   BOOST_FOREACH (string_pair const& ex, average_expressions) {
    average_values[ex.first].push_back(iter++->value());
  }
...
dmtk::BasicOp<double> DMRGTask::create_site_operator(std::string const& name, alps::SiteOperator const& siteop, int type)
{ 
  dmtk::Block<double> &block = site_block[type];
  dmtk::BasicOp<double> *op = block(name.c_str(),0);
  if(!op) {
    ...
    dmtk::Matrix<double> dest(orig.shape()[0],orig.shape()[1]);
    dmtk::QN dqn;
    bool first = true;
    for(size_t i = 0; i < orig.shape()[0]; i++){
      for(size_t j = 0; j < orig.shape()[1]; j++){
        dest[j][i] = orig[i][j];
        if(fabs(dest[j][i]) > 1.e-10){
          dmtk::QN new_dqn;
    ...
    dmtk::BasicOp<double> new_op(name.c_str(),0);
    new_op.dqn = dqn;
...
void
DMRGTask::build_site_operator(alps::SiteOperator const& siteop, int site, dmtk::Hami<double> &this_hami)
{
  typedef std::vector<boost::tuple<alps::Term,alps::SiteOperator> > V;
  V  ops = siteop.split();
  int type = site_type(site);
  alps::expression::ParameterEvaluator<std::complex<double> > coords(coordinate_as_parameter(site));
  for (V::iterator it=ops.begin(); it!=ops.end();++it) {
    std::string name = simplify_name(it->get<1>());
    dmtk::BasicOp<double> op1(name.c_str(),site);
    op1 = create_site_operator(name,it->get<1>(),type);
    op1.set_site(site);
    dmtk::Term<double> real_t = op1;
                            ...
                            std::pair<int,int> sites, dmtk::Hami<double> &this_hami)
{
  ...
  for (V::const_iterator tit1=ops1.begin(); tit1!=ops1.end();++tit1)
    for (V::const_iterator tit2=ops2.begin(); tit2!=ops2.end();++tit2) {
      ...
      dmtk::BasicOp<double> op1(s_name1.c_str(),sites.first); 
      dmtk::BasicOp<double> op2(s_name2.c_str(),sites.second); 
      ...
      dmtk::Term<double> real_t = op1*op2;
...
void
DMRGTask::build_bond_operator(alps::BondOperator const& bondop, bond_descriptor const& b, dmtk::Hami<double> &this_hami)
{
  typedef std::vector<boost::tuple<alps::Term,alps::SiteOperator,alps::SiteOperator > > V;
  alps::expression::ParameterEvaluator<std::complex<double> > coords(coordinate_as_parameter(b));
  ...
  for (V::iterator tit=ops.begin(); tit!=ops.end();++tit) {
    ...
    dmtk::BasicOp<double> op1(s_name1.c_str(),source(b)); 
    dmtk::BasicOp<double> op2(s_name2.c_str(),target(b)); 
