...
<xsl:template match="/">
  <xslout:stylesheet version="1.0">
    ...
    <xslout:template name="CheckConstraints"><xsl:value-of select="$newline"/>
        ...
        <xslout:for-each select="//SIMULATION">
           ...
           <xsl:text disable-output-escaping = "yes">&lt;!-- global loop constraints --&gt;</xsl:text>
           <xsl:value-of select="$newline"/>
           ...
           <xslout:call-template name="CheckSetConstraints"/>
        </xslout:for-each>
      ...
      <xslout:template name="CheckSetConstraints"><xsl:value-of select="$newline"/>
        ...
        <xsl:for-each select="//constraint">
          <xsl:if test="@condition">
            <xsl:if test="@type = 'PARAMETER' or @type = 'SCALAR_AVERAGE'">
      ...
      </xslout:template><!-- CheckSetConstraints -->
