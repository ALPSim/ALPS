...
int main(int argc, char *argv[]) {
    ...
    if (options.resume)
        sim.load(params.value_or_default("DUMP", "dump") + "." + boost::lexical_cast<std::string>(color));

    sim.run(boost::bind(&basic_stop_callback, options.time_limit));

    sim.save(params.value_or_default("DUMP", "dump") + "." + boost::lexical_cast<std::string>(color));
