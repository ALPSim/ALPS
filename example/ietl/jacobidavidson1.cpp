...
int main(int argc, char **argv)
{
    ...
    // on default GMRES with 5 steps is used
    ...
    // the correction equation has to be solved exactly, which is expensive. 
    // here we use different starting vectors instead.
    try {
            jd.eigensystem(iter, gen, n_evals);
            jd.eigensystem(iter, gen2, n_evals-n_evals/2);
    } catch (std::runtime_error& e) {
            cerr << "Error in eigenpair calculation: \n";
            cout << e.what() << "\n";
            return 1;
    }
