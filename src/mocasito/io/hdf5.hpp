		...
		class hdf5 {
			public:
				...
				type_traits<>::type attrtype(std::string const & p, std::string const & s) const {
					MOCASITO_TRACE
					if (is_group(p))
						return attrtype_helper<detail::h5g_t>(H5Gopen2(_file, p.c_str(), H5P_DEFAULT), p, s);
					else
						return attrtype_helper<detail::h5g_t>(H5Dopen2(_file, p.c_str(), H5P_DEFAULT), p, s);
				}
				type_traits<>::type datatype(std::string const & p) const {
					detail::h5d_t data_id(H5Dopen2(_file, p.c_str(), 0));
			...
			private:
				...
				template<typename I> type_traits<>::type attrtype_helper(I const & data_id, std::string const & p, std::string const & s) const {
					MOCASITO_TRACE
