...
The prerequisites for this installation are listed in Section~\ref{sct:build_and_install}. For some widely used Linux distributions, we describe the installation of these prerequisites on the website of the ALPS project; for other cases, please refer to your system documentation. Note also that Boost does not have to be compiled to build ALPS.

The CMake build process is controlled by a set of variables describing the build environment, paths, and targets (libraries and programs) to be built. Most of these variables are determined in the global file {\tt CMakeLists.txt}. Each source directory also contains such a file, which usually contain prescriptions for building certain targets given the global configuration variables. A major advantage of CMake over other build systems such as the {\tt autotools} is that it allows the user easy access and modification of all variables via a graphical interface, the tool {\tt ccmake} or by directly editing the file {\tt CMakeCache.txt}.
...
It is strongly recommended to perform the configuration and compilation of ALPS in a separate directory and not the source directory. The procedure for configuring and installing ALPS is given below, where we assume that all commands are run from the build directory:
\begin{verbatim}
...
make install
\end{verbatim}
The options starting with {\tt -D} are used to pass variables that are needed in the initial stages of CMake; cf. Table~\ref{table:cmake_variables}. Running {\tt ccmake} after this is not mandatory and can be used to change all other variables, including those described in Table~\ref{table:cmake_variables2}.
...
{\tt CMAKE\_C\_COMPILER} &{\tt C} compiler. This is determined from the environment variables {\tt CC}. \\ \hline
{\tt CMAKE\_CXX\_COMPILER} &{\tt C++} compiler. This is determined from the environment variables {\tt CXX}. \\ \hline
{\tt PYTHON\_INTERPRETER} &Path of the Python interpreter that {\tt pyalps} should be built for. If not set, a default or will be determined or Python disabled. \\ \hline
\end{tabular}
...
\hline
{\it Name} &{\it Description} \\ \hline
{\tt CMAKE\_INSTALL\_PREFIX} & \\ \hline
{\tt VISTRAILS\_APP\_DIR} & \\ \hline
