...
simulations, sparse and full diagonalization (ED), as well as the
density matrix renormalization group (DMRG) for static problems and the
time-evolving block decimation (TEBD) algorithm for time-dependent
simulations, and quantum Monte Carlo solvers for dynamical mean field theory (DMFT). Major changes in release 2.0 include the use of HDF5 for binary data, evaluation tools in Python, support for Windows  operating system, the use of CMake as build system and binary installation packages for Mac OS X and Windows, as well as integration with the VisTrails workflow provenance tool.
