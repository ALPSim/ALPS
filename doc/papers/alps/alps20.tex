...
function $\Sigma(\omega)$. For such a ``local'' self-energy, the diagrammatics simplify radically and the problem may be mapped onto an impurity problem
...
create it. Opening that workflow lets the reader execute the full calculation. >>>SHOULD ERROR BARS NOT BE BETTER INCLUDED IN THIS EXAMPE PLOT?<<<}
\label{fig:figure}
\end{center}
\end{figure}
...
>>SHOULD ONE NOT BETTER INCLUDE ERROR BARS IN THIS EXAMPLE?<<}
\label{fig:python}
\end{figure}
