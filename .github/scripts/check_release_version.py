...
def check_distributions(directory: Path, expected: Version) -> None:
    ...
    for artifact in artifacts:
        if artifact.suffix == ".whl":
            name, version, _, tags = parse_wheel_filename(artifact.name)
        ...
        if artifact.suffix == ".whl" and any(
            tag.interpreter != "cp312" or tag.abi != "abi3" or tag.platform == "any"
            for tag in tags
        ):
            raise ValueError(f"{artifact.name}: expected a platform-specific cp312-abi3 wheel")
