...
def test_external_executable_with_dll_only_package(tmp_path, monkeypatch):
    """Disabling CLI bundling must still allow an explicitly supplied executable."""
    import pyalps.tools as tools

    binary = tmp_path / "bin"
    binary.mkdir()
    (binary / "alps.dll").touch()
    monkeypatch.setattr(tools, "_packaged_dir", lambda name: str(binary))
    monkeypatch.setenv("PATH", "")
    monkeypatch.setenv("ALPS_BIN_PATH", str(Path(sys.executable).parent))
    tools.check_existence(Path(sys.executable).name)
...
def test_native_parameter_contracts(tmp_path):
    ...
    completed = subprocess.run(
        ...
        env={**os.environ, "PYTHONPATH": os.pathsep.join(
            filter(None, (str(build), os.environ.get("PYTHONPATH")))), "MallocScribble": "1"},
