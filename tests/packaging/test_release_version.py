...
def test_prerelease_sdist_keeps_its_version_without_the_build_environment(tmp_path):
    ...
    assert (unpacked / "_build_support/pyalpsConfig.cmake").is_file()
    assert (unpacked / "cpp/export_simulation.hpp").is_file()
    assert (unpacked / "examples/ising/CMakeLists.txt").is_file()
    assert (unpacked / "examples/ising/export2py.cpp").is_file()
    for binding in ("maxent_c", "cthyb", "ctint", "params", "hdf5", "mcbase"):
...
@pytest.mark.parametrize("tag", [
    "cp312-cp312-win_amd64", "cp313-abi3-win_arm64", "py3-none-any", "cp312-abi3-any",
])
def test_rejects_incorrect_wheel_abi(tmp_path, artifact, tag):
    artifact("wheel", tag=tag)
    with pytest.raises(ValueError, match="expected a platform-specific cp312-abi3 wheel"):
